...
class TransformerBlock(nn.Module): 
    ...
    def attention(self, x, x_condition=None): 
...
class Transformer(nn.Module): 
    def __init__(
        ...
        self.layers = nn.ModuleList([TransformerBlock(n_embed, n_heads, attn_mask=attn_mask, mlp_scale=mlp_scale, attn_drop=attn_drop, resid_pdrop=resid_pdrop) for _ in range(n_layers)]) 
...
class ConditionedGPT(nn.Module):
    ...
    def forward(self, x, y=None, x_condition=None):
        x = self.embed(x)
        x = self.transformer(x, x_condition)
        x = self.ln_post(x)
    ...
    def sample(self, x, x_condition, temperature=1.0, top_k=40, max_length=100, batch_size=1):
        ...
        condition_batch = x_condition.repeat_interleave(batch_size, dim=0)
